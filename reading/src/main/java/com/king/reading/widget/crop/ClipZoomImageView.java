...
public class ClipZoomImageView extends android.support.v7.widget.AppCompatImageView implements
    ...
    public boolean onScale(ScaleGestureDetector detector) {
        ...
        int innerWidth = width - 2 * mHorizontalPadding;

        float picNewsWidth = rect.right - rect.left;
        float picNewHeight = rect.bottom - rect.top;
        float minumSize = Math.min(picNewsWidth, picNewHeight);

        Log.d(TAG, "scale" + scale);
        Log.d(TAG, "scaleFactor" + scaleFactor);
        Log.d(TAG, "innerWidth/minumSize " + innerWidth / minumSize);
        ...
        float factor = innerWidth / minumSize;
    ...
    private void checkBorder() {
        ...
        // 如果宽或高大于屏幕，则..0围 ; 这里的0.001是因为精度丢失会产生问题，但是误差一般很小，所以我们直接加了一个0.01
     ...
     */
    public Bitmap clip() {
        Bitmap bitmap = Bitmap.createBitmap(Math.max(0, getWidth()), Math.max(0, getHeight()),
                Bitmap.Config.RGB_565);
        Canvas canvas = new Canvas(bitmap);
        draw(canvas);
        return Bitmap.createBitmap(bitmap, Math.max(0, mHorizontalPadding),
                Math.max(0, mVerticalPadding), Math.max(0, getWidth() - 2 * mHorizontalPadding),
                Math.max(0, getHeight() - 2 * mVerticalPadding));
    }
    ...
    public void onGlobalLayout() {
        if (once) {
            ...
            Log.d(TAG, "deltaScale :::   onGlobalLayout");

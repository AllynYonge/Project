...
package com.king.reading.widget.crop.cropwindow;
...
import com.king.reading.widget.crop.cropwindow.handle.Handle;
import com.king.reading.widget.crop.util.AspectRatioUtil;
import com.king.reading.widget.crop.util.HandleUtil;
import com.king.reading.widget.crop.util.PaintUtil;
...
//            setLayerType(View.LAYER_TYPE_SOFTWARE, null);
    ...
    private void initCropWindow(Rect bitmapRect) {
        ...
        if (CropImageView.CropMode.FIXCROPAREA.equals(mCropMode)){
            if (mCropShape.equals(CropImageView.CropShape.RECTANGLE)){
                Edge.TOP.setCoordinate((bitmapRect.height() - (mCropWidth * mAspectRatioY / mAspectRatioX))/2);
                Edge.BOTTOM.setCoordinate(bitmapRect.height() - Edge.TOP.getCoordinate());
                Edge.LEFT.setCoordinate(0);
                Edge.RIGHT.setCoordinate(bitmapRect.width());
            } else if (mCropShape.equals(CropImageView.CropShape.OVAL)){
                Edge.TOP.setCoordinate((bitmapRect.height() - (mCropWidth * mAspectRatioY / mAspectRatioX))/2);
                Edge.BOTTOM.setCoordinate(Edge.TOP.getCoordinate() + mCropWidth);
                Edge.LEFT.setCoordinate((bitmapRect.width() - mCropWidth) / 2);
                Edge.RIGHT.setCoordinate(Edge.LEFT.getCoordinate() + mCropWidth);
            }
            return;
        }
        ...
        if (mFixAspectRatio
            ...
            if (AspectRatioUtil.calculateAspectRatio(bitmapRect) > mTargetAspectRatio) {
                ...
                final float cropHeight = Math.max(Edge.MIN_CROP_LENGTH_PX,
                        //根据mAspectRatioX) / mAspectRatioY算出比率，然后根据比率结合宽度，算出高度
            ...
            //若mFixAspectRatio == false,就走这个逻辑，默认裁剪区域为图像的90%的大小
